function D3Trees.D3Tree(p::SSUOPlanner; title="SSUO Tree", kwargs...)
    @warn("""
         D3Tree(planner::SSUOPlanner) is deprecated and may be removed in the future. Instead, please use
    ...
    if p.tree == nothing
        error("SSUOPlanner has not constructed a tree yet, run `action(planner, belief)` first to construct the tree.")
    end
    return D3Tree(p.tree; title=title, kwargs...)
end

function D3Trees.D3Tree(t::SSUOTree; title="SSUO Tree", kwargs...)
...
Base.show(io::IO, mime::MIME"text/html", t::SSUOTree) = show(io, mime, D3Tree(t))
Base.show(io::IO, mime::MIME"text/plain", t::SSUOTree) = show(io, mime, D3Tree(t))
