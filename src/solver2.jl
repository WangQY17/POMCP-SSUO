function simulate(pomcp::SSUOPlanner, h_node::SSUOTreeObsNode{B,A,O}, s::S, d) where {B,S,A,O}
    tree = h_node.tree
    h = h_node.node
    sol = pomcp.solver
    dep = 1.0
    ...
    if sol.enable_action_pw
        ...
        if length(tree.tried[h]) <= sol.k_action * total_n ^ sol.alpha_action / (max_depth + 1 - d)
        # if length(tree.tried[h]) <= 4 * sqrt(total_n) / (max_depth + 1 - d)
            if h == 1
                a = next_action(pomcp.next_action, pomcp.problem, tree.root_belief, SSUOTreeObsNode(tree, h))
            else
                a = next_action(pomcp.next_action, pomcp.problem, StateBelief(tree.sr_beliefs[h]), SSUOTreeObsNode(tree, h))
            end
            if !sol.check_repeat_act || !haskey(tree.o_child_lookup, (h,a))
                push_anode!(tree, h, a,
                            init_N(pomcp.init_N, pomcp.problem, SSUOTreeObsNode(tree, h), a),
                            init_V(pomcp.init_V, pomcp.problem, SSUOTreeObsNode(tree, h), a),
    ...
    if r == Inf
        @warn("SSUO: +Inf reward. This is not recommended and may cause future errors.")
    end

    if new_node
        dep = dep + 1.0
        R = r + POMDPs.discount(pomcp.problem)*estimate_value(pomcp.solved_estimate, pomcp.problem, sp, SSUOTreeObsNode(tree, hao), d-1)
    else
        ...
        reward, depth = simulate(pomcp, SSUOTreeObsNode(tree, hao), sp, d-1)
        dep = depth + 1
        # dep = max(depth + 1, dep)
        R = r + POMDPs.discount(pomcp.problem) * reward
        # R = r + POMDPs.discount(pomcp.problem)*simulate(pomcp, SSUOTreeObsNode(tree, hao), sp, d-1)
    end
