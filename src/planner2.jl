mutable struct SSUOPlanner{P,NBU,C,NA,SE,IN,IV,SolverType} <: Policy
    solver::SolverType
    problem::P
    ...
    init_N::IN
    init_V::IV
    tree::Union{Nothing, SSUOTree} # this is just so you can look at the tree later
end

function SSUOPlanner(solver, problem::POMDP)
    SSUOPlanner(solver,
                  problem,
...
Random.seed!(p::SSUOPlanner, seed) = Random.seed!(p.solver.rng, seed)

function action_info(pomcp::SSUOPlanner{P,NBU}, b; tree_in_info=false) where {P,NBU}
    A = actiontype(P)
...
action(pomcp::SSUOPlanner, b) = first(action_info(pomcp, b))

function POMDPPolicies.actionvalues(p::SSUOPlanner, b)
    tree = make_tree(p, b)
    search(p, tree)
...
function make_tree(p::SSUOPlanner{P, NBU}, b) where {P, NBU}
    S = statetype(P)
    A = actiontype(P)
    O = obstype(P)
    B = belief_type(NBU,P)
    return SSUOTree{B, A, O, typeof(b)}(b, 2*min(100_000, p.solver.tree_queries))
    # return SSUOTree{B, A, O, typeof(b)}(b, 2*p.solver.tree_queries)
end


function search(pomcp::SSUOPlanner, tree::SSUOTree, info::Dict{Symbol,Any}=Dict{Symbol,Any}())
    all_terminal = true
    # gc_enable(false)
    i = 0
    start_us = CPUtime_us()
    ...
    while i < pomcp.solver.tree_queries
        ...
        if !POMDPs.isterminal(pomcp.problem, s)
            ...
            reward, depth = simulate(pomcp, SSUOTreeObsNode(tree, 1), s, max_depth)
    ...
    best_node = select_best(pomcp.solver.final_criterion, SSUOTreeObsNode(tree,1), pomcp.solver.rng)
