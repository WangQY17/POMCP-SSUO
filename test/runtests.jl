...
@testset "all" begin
    ...
    @testset "POMDPTesting" begin
        ...
        solver = SSUOSolver(max_time=0.1, tree_queries=typemax(Int))
        test_solver(solver, pomdp, updater=DiscreteUpdater(pomdp))
    end
    ...
    @testset "type stability" begin
        ...
        B = SSUO.belief_type(SSUO.SSUONodeFilter, typeof(pomdp))
        tree = SSUOTree{B,Bool,Bool,typeof(b)}(b, 2*planner.solver.tree_queries)
        @inferred SSUO.simulate(planner, SSUO.SSUOTreeObsNode(tree, 1), true, 10)

        pomdp = LightDark1D()
        solver = SSUOSolver(default_action=485)
        planner = solve(solver, pomdp)
